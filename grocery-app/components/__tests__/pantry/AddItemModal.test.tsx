...
describe('AddItemModal', () => {
    ...
    const mockSetBarcodeScanning = jest.fn();
    ...
    const setup = (visible: boolean, initialValues = {}) => {
        return renderWithProviders(
            <AddItemModal
                ...
                setBarcodeScanning={mockSetBarcodeScanning}
            />
        );
    };
    ...
    it('adds new nutrient correctly', () => {
        const { getByText, queryByText } = setup(true);

        fireEvent.press(getByText('Nutrition Info'));
        expect(queryByText('Add Nutrients')).toBeTruthy();

        fireEvent.press(getByText('Nutrition Info'));
        expect(queryByText('Add Nutrients')).toBeNull();

        // expect(getByText('Protein')).toBeTruthy();
        // expect(getByDisplayValue('10g')).toBeTruthy();
    });

    it('toggles nutrition section visibility', () => {
        const { getByText, queryByText } = setup(true);
    ...
    it('calls setBarcodeScanning with false when item is added', async () => {
        (axios.post as jest.Mock).mockResolvedValueOnce({});
        ...
        await waitFor(() => {
            expect(mockSetBarcodeScanning).toHaveBeenCalledWith(false);
        });
    });
});
